...
public class StyledTextExample {
	public static void main(String[] args) {
		...
		text.setText("The quick brown fox jumps over the lazy dog."
				+ "\nThat's all folks!");
		TextStyle textStyle = new TextStyle(new Font(display, "Courier", 12,
				SWT.BOLD), display.getSystemColor(SWT.COLOR_RED), null);
		textStyle.strikeout = true;
		textStyle.underline = true;
		textStyle.underlineStyle = SWT.UNDERLINE_SINGLE;
		text.setStyleRanges(new int[] { 4, 5 },
				new StyleRange[] { new StyleRange(textStyle) });

		text.getAccessible().addAccessibleEditableTextListener(
				new AccessibleEditableTextAdapter() {
					@Override
